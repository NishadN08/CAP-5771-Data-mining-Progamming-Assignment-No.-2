...
def fit_kmeans(X, k):
    ...
    for clusters_num in range(1, k + 1):
        ...
        for iter, prediction in enumerate(preds):
            alp = (X[iter][0] - kmeans.cluster_centers_[prediction][0]) ** 2 + (
                        X[iter][1] - kmeans.cluster_centers_[prediction][1]) ** 2
            try:
                sse[prediction] += alp
            except:
                sse[prediction] = alp
...
def compute():
    ...
    centers = 5
    random_state = 12
    x, label = datasets.make_blobs(n_samples=n_samples, centers=centers, center_box=center_box,
